...
func _ready() -> void:
	SignalBus.camera_shake_requested.connect(_on_camera_shake)
	# 初始化目标缩放
	target_zoom = zoom
	# 确保摄像机不会超出限制
...
func _on_camera_shake(intensity:float,duration:float):
	trauma = clamp(intensity+duration,0.0,1.0)

func _apply_shake(delta:float):
	var time = Time.get_ticks_msec()
	var low_freq = noise.get_noise_1d(time*10)*8
	var high_freq = noise.get_noise_1d(time*50)*4
	offset = Vector2(low_freq+high_freq,low_freq+high_freq)*trauma
